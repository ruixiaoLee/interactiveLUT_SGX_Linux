...
int main(int argc, char *argv[]){
  ...
  table_size_out = pow(2,bit_num*2);
  ...
  vector<Ciphertext> ct_result1, ct_result2, ct_result3;
  vector<vector<int64_t> > dec_result1(row_count);
  vector<vector<int64_t> > dec_result2(row_count);
  vector<vector<int64_t> > dec_result3(row_count);
  vector<Plaintext> poly_dec_result1, poly_dec_result2, poly_dec_result3;
  for(int s=0 ; s<row_count ; s++){
    Plaintext ex;
    poly_dec_result1.push_back(ex);
    poly_dec_result2.push_back(ex);
    poly_dec_result3.push_back(ex);
  }
  cout << "Reading intermediate result..." << flush;
  ifstream mid_result1("../Result/IntermediateRes1_"+bit_num_str, ios::binary);
  ifstream mid_result2("../Result/IntermediateRes2_"+bit_num_str, ios::binary);
  ifstream mid_result3("../Result/IntermediateRes3_"+bit_num_str, ios::binary);
  Ciphertext temp1;
  ...
  Ciphertext temp2;
  for(int w = 0; w < row_count ; w++) {
    temp2.load(context, mid_result2);
    ct_result2.push_back(temp2);
  }
  mid_result2.close();
  Ciphertext temp3;
  for(int w = 0; w < row_count ; w++) {
    temp3.load(context, mid_result3);
    ct_result3.push_back(temp3);
  }
  mid_result3.close();
  cout << "OK" << endl;
  ...
  for (int z=0 ; z<row_count ; z++){
    ...
    decryptor.decrypt(ct_result2[z], poly_dec_result2[z]);
    batch_encoder.decode(poly_dec_result2[z], dec_result2[z]);
    decryptor.decrypt(ct_result3[z], poly_dec_result3[z]);
    batch_encoder.decode(poly_dec_result3[z], dec_result3[z]);
  }
  cout << "OK" << endl;
  ...
  int64_t index_row_z,index_col_z;
  int64_t flag3=0;

  for(int i=0 ; i<row_count ; i++){
    for(int j=0 ; j<row_size ; j++){
      if(dec_result3[i][j] == 0 && flag3 == 0){
        index_row_z = i;
        index_col_z=j;
        flag3 = 1;
        break;
  ...
  if(flag2 == 0) cout<<"ERROR 2: NO FIND INPUT NUMBER!"<<endl;
  if(flag3 == 0) cout<<"ERROR 2: NO FIND INPUT NUMBER!"<<endl;
  cout << "OK" << endl;
  cout<<"x[row,col]=["<<index_row_x<<", "<<index_col_x<<"],"
      <<"y[row,col]=["<<index_row_y<<", "<<index_col_y<<"],"
      <<"z[row,col]=["<<index_row_z<<", "<<index_col_z<<"]"<<endl;
  ...
  if(n_query==1){
    ...
    int64_t index = (index_row_x * row_size + index_col_x) * table_size_in * table_size_in
                     + (index_row_y * row_size + index_col_y) * table_size_in
                     + (index_row_z * row_size + index_col_z);
